...
- Filtra TO=Baobab e APT richiesto (opzionale)
...
Esempi:
  python consuntivobaobab.py -i "Piano lavoro DICEMBRE 25.xlsx" -o "OUT_BAOBAB.xlsx"
  python consuntivobaobab.py -i "Piano lavoro DICEMBRE 25.xlsx" -o "OUT_BAOBAB.xlsx" --apt VRN
"""
...
# Tariffe base Baobab per 2h30 (150 min)
TARIFFE_BASE_BAOBAB = {
    "BGY": 80.0,
    "MXP": 85.0,
    "LIN": 85.0,
...
# Tariffe notturne Baobab (€/min) - 20% della tariffa base oraria
TARIFFE_NOTTE_BAOBAB = {
    "BGY": 0.1067,  # 80€/2.5h = 32€/h, 20% = 6.40€/h = 0.1067€/min
    "MXP": 0.1133,  # 85€/2.5h = 34€/h, 20% = 6.80€/h = 0.1133€/min
    "LIN": 0.1133,
    "VRN": 0.1133,
    "BLQ": 0.1133,
    "TRN": 0.1133,
    ...
    "CTA": 0.1200,
    "PMO": 0.1200,
    "FCO": 0.1267,  # 95€/2.5h = 38€/h, 20% = 7.60€/h = 0.1267€/min
...
class CalcConfig:
    ...
    to_keyword: str = "baobab"  # Default, ma accetta anche "th" (Baobab e TH sono lo stesso pacchetto)
    ...
    # Tariffe Baobab/TH (stesso pacchetto)
...
def night_minutes(start_dt: pd.Timestamp, end_dt: pd.Timestamp) -> int:
    """
    Calcola minuti notturni nella fascia 22:00-06:00 (Baobab)
...
def get_base_tariffa(apt: str) -> float:
    ...
    return TARIFFE_BASE_BAOBAB.get(apt_upper, 85.0)  # Default: 85€


def get_night_tariffa(apt: str) -> float:
    """Restituisce la tariffa notturna per minuto per l'aeroporto"""
    apt_upper = apt.upper().strip()
    return TARIFFE_NOTTE_BAOBAB.get(apt_upper, 0.1133)  # Default: MXP/VRN rate
...
def detect_columns(df: pd.DataFrame) -> Dict[str, Optional[str]]:
    ...
    return {
        ...
        "turni": find_col(df, [r"^TURNI$", r"\bTURNI\b"]),
    }
...
def process_files(input_files: List[str], cfg: CalcConfig) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """
    Legge e processa i file Piano Lavoro per Baobab
    
    Returns: (detail_blocks_df, totals_df, discrepancies_df)
    Per ora solo lettura, senza calcoli (da implementare secondo regole Baobab)
    """
            ...
            for idx, r in sdf.iterrows():
                ...
                if cols["tour_operator"] and cols["tour_operator"] in r.index:
                    ...
                    if to_val and to_val.lower() not in ["nan", "none", ""]:
                        # Normalizza: se contiene "th" (non "baobab"), usa "TH", altrimenti mantieni originale
                        if "th" in to_val.lower() and "baobab" not in to_val.lower():
                            tour_operator_orig = "TH"
                        elif "baobab" in to_val.lower() and "th" not in to_val.lower():
                            tour_operator_orig = "Baobab"
                        elif "baobab/th" in to_val.lower() or "baobab / th" in to_val.lower():
                            tour_operator_orig = "Baobab/TH"
                        else:
                ...
                if cols["compagnia"] and cols["compagnia"] in r.index:
                    comp_val = str(r[cols["compagnia"]]).strip()
                    if comp_val and comp_val.lower() not in ["nan", "none", ""]:
                        compagnia_val = comp_val
                ...
                cvc_minuti = 0
                # Cerca in NOTE
                if cols["note"] and cols["note"] in r.index:
                    note_val = str(r[cols["note"]]).strip()
                    if note_val and note_val.lower() not in ["nan", "none", ""]:
                        # Cerca pattern CVC seguito da numero (es. CVC30, CVC60, cvc30, cvc60)
                        match = re.search(r"cvc\s*(\d+)", note_val, re.IGNORECASE)
                        if match:
                            cvc_minuti = int(match.group(1))
                
                # Se non trovato in NOTE, cerca in TURNO/TURNI
                if cvc_minuti == 0:
                    turno_val = turno_ffill_raw
                    if turno_val and turno_val.lower() not in ["nan", "none", ""]:
                        match = re.search(r"cvc\s*(\d+)", turno_val, re.IGNORECASE)
                        if match:
                            cvc_minuti = int(match.group(1))
                
                # Se non trovato in TURNO_FFILL, cerca anche in TURNI se presente
                if cvc_minuti == 0 and cols["turni"] and cols["turni"] in r.index:
                    turni_val = str(r[cols["turni"]]).strip()
                    if turni_val and turni_val.lower() not in ["nan", "none", ""]:
                        match = re.search(r"cvc\s*(\d+)", turni_val, re.IGNORECASE)
                        if match:
                            cvc_minuti = int(match.group(1))
    ...
    for key, b in sorted(blocks.items(), key=lambda kv: (kv[1].date, kv[1].apt, kv[1].first_source.original_order)):
        # BAOBAB: START = STD - 2:30 (normale)
        # Per Wizzair a Fiumicino (FCO): START = STD - 3:00 (la mezzora aggiuntiva va conteggiata come extra)
        ...
        if std_sel is None:
            ...
            start_baobab = b.start_dt
            errore_msg = "STD non disponibile, uso TURNO come fallback"
        else:
            # START = STD - 3:00 per Wizzair a FCO, altrimenti STD - 2:30
            if is_wizzair_fco:
                start_baobab = std_sel - pd.Timedelta(hours=3, minutes=0)
                errore_msg = None
            else:
                start_baobab = std_sel - pd.Timedelta(hours=2, minutes=30)
                errore_msg = None
        ...
        # Verifica che start_baobab e end_baobab siano validi
        if pd.isna(start_baobab) or pd.isna(end_baobab):
            ...
            durata_effettiva_min = int((end_baobab - start_baobab).total_seconds() / 60.0)
            
            # Turno base (sempre 2h30 = 150 min)
            turno_eur = compute_turno_eur(b.apt)
            
            # Extra = MAX(0, durata_effettiva - 150) + minuti CVC (mezzora anticipata)
            # I minuti CVC (es. CVC30, CVC60) vengono aggiunti come extra
            extra_min_base = max(0, durata_effettiva_min - cfg.durata_base_min)
            extra_min = extra_min_base + b.cvc_minuti_extra
            extra_eur = extra_min * (cfg.rate_extra_per_h / 60.0)
            
            # Notturno (22:00-06:00)
            night_min = night_minutes(start_baobab, end_baobab)
            ...
            if is_festivo:
                totale = (turno_eur + extra_eur + night_eur) * cfg.festivo_multiplier
            else:
                totale = turno_eur + extra_eur + night_eur
        ...
        rows_detail.append({
            ...
            "EXTRA_MIN": extra_min,
            "EXTRA_MIN_CVC": b.cvc_minuti_extra,  # Minuti extra da CVC
    ...
    # Baobab e Baobab/TH sono lo stesso pacchetto, normalizziamo per i totali
    detail_df["__TO_NORMALIZED"] = detail_df["TOUR OPERATOR"].astype(str).apply(
        lambda x: "Baobab/TH" if ("baobab" in x.lower() or "th" in x.lower()) else x
    )
    ...
    # Riga totale mese (combinato per Baobab e Baobab/TH)
    # Per il totale mese, usiamo "Baobab/TH" come nome unificato
    month_row = pd.DataFrame([{
        "TOUR OPERATOR": "Baobab/TH" if "TOUR OPERATOR" in detail_df.columns else "",
        "PERIODO": "MESE",
...
def create_apt_detail_sheet(df_apt: pd.DataFrame) -> pd.DataFrame:
    ...
    # Add Compagnia and Assistente columns if present (insert after Tour Operator)
    new_cols = {'Data': output_cols['Data']}
    new_cols['Tour Operator'] = output_cols['Tour Operator']
    
    if 'COMPAGNIA' in df_apt.columns:
        new_cols['Compagnia'] = df_apt['COMPAGNIA'].fillna('')
    ...
    # Add empty Compagnia and Assistente in total row if columns exist
    if 'COMPAGNIA' in df_apt.columns or 'ASSISTENTE' in df_apt.columns:
        new_total_dict = {'Data': total_row_dict['Data']}
        new_total_dict['Tour Operator'] = total_row_dict['Tour Operator']
        if 'COMPAGNIA' in df_apt.columns:
            new_total_dict['Compagnia'] = ''
        if 'ASSISTENTE' in df_apt.columns:
    ...
    with pd.ExcelWriter(output_path, engine="openpyxl", datetime_format="YYYY-MM-DD HH:MM") as writer:
        ...
        if not detail_df.empty:
            cols = [
                ...
                "TURNO_EUR",
                "EXTRA_MIN", "EXTRA_MIN_CVC", "EXTRA_EUR",
                "NOTTE_MIN", "NOTTE_EUR",
...
def main():
    parser = argparse.ArgumentParser(description="Calcolatore blocchi Baobab")
    ...
    parser.add_argument("--to", default="baobab", help="Keyword tour operator (default: baobab)")
