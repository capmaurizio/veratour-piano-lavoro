...
# ── Tariffe IOT ──────────────────────────────────────────────────────────────
TARIFFE_BASE = {
    "BGY":110.0,"VRN":110.0,"BLQ":110.0,"LIN":110.0,"MXP":110.0,
    "TRN":110.0,"NAP":110.0,"BRI":110.0,"CTA":110.0,"PMO":110.0,
    "VCE":180.0,"FCO":170.0,"PSA":170.0,
}
...
class CalcConfig:
    ...
    rate_extra_per_h: float = 20.0
    durata_base_min: int = 150
    festivo_multiplier: float = 1.30
    include_4oct: bool = True  # Flyness include 4 ottobre
...
def process_files(input_files:List[str], cfg:CalcConfig):
    ...
    for key,b in sorted(blocks.items(),key=lambda kv:kv[1].first_source.original_order if kv[1].first_source else 0):
        ...
        # Turno base
        tariff=TARIFFE_BASE.get(b.apt.upper().strip(),110.0)
        turno_eur=tariff
        ...
        notte_rate=TARIFFE_NOTTE.get(b.apt.upper().strip(),round(110.0/150*0.20,6))
        ...
        rows.append({
            ...
            "APT":b.apt,"TOUR OPERATOR":"Flyness",
