...
"""
Caboverdetime — Calcolatore blocchi

Cosa fa:
- Legge file Excel "Piano Lavoro" (anche con più fogli)
- Filtra TO=Caboverdetime e APT richiesto (opzionale)
...
Esempi:
  python consuntivocaboverdetime.py -i "Piano lavoro DICEMBRE 25.xlsx" -o "OUT_CABOVERDETIME.xlsx"
  python consuntivocaboverdetime.py -i "Piano lavoro DICEMBRE 25.xlsx" -o "OUT_CABOVERDETIME.xlsx" --apt VRN
"""
...
class CalcConfig:
    ...
    to_keyword: str = "caboverdetime"  # Default
    ...
    # Tariffe Caboverdetime
    rate_extra_per_h: float = 18.0  # €18/h = €0.30/min
    durata_base_min: int = 180  # 3 ore = 180 minuti
...
def night_minutes(start_dt: pd.Timestamp, end_dt: pd.Timestamp) -> int:
    """
    Calcola minuti notturni nella fascia 22:00-06:00 (Caboverdetime)
    ...
    while current < end_dt:
        ...
        # Fascia: 22:00-06:00 (Caboverdetime)
        is_night = False
        if hour == 22:
            is_night = True
        elif hour == 23:
...
def process_files(input_files: List[str], cfg: CalcConfig) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """
    Legge e processa i file Piano Lavoro per Caboverdetime
    ...
    for file_path in input_files:
        for sheet_name, sdf0 in iter_excel_sheets(file_path):
            ...
            for idx, r in sdf.iterrows():
                ...
                # Caboverdetime non ha spese parcheggio
                ...
                # Extract CONVOCAZIONE (CVC) from row - Caboverdetime
                cvc_dt = None
                ...
                # CABOVERDETIME: ogni riga del Piano Lavoro genera un blocco separato
    ...
    for key, b in sorted(blocks.items(), key=lambda kv: (kv[1].date, kv[1].apt, kv[1].first_source.original_order)):
        # CABOVERDETIME: Base = da CVC (convocazione) a STD
        # Extra = da STD a ATD
        # Notturno = calcolato su tutto l'intervallo (CVC a ATD o fine turno)
        ...
        # Per Caboverdetime, assicuriamoci che STD sia nella data corretta della riga
        ...
        # Verifica che CVC e STD siano disponibili per il calcolo base
        if pd.isna(cvc_dt) or pd.isna(std_sel):
            errore_msg = "CVC o STD non disponibili"
            turno_eur = 0.0
            extra_min = 0
            extra_eur = 0.0
            night_min = 0
            ...
            if pd.notna(end_for_night):
                night_min = night_minutes(cvc_dt, end_for_night)
            ...
            durata_base_min = int((std_sel - cvc_dt).total_seconds() / 60.0) if pd.notna(std_sel) and pd.notna(cvc_dt) else 0
            start_caboverdetime = cvc_dt
            end_caboverdetime = atd_sel if pd.notna(atd_sel) else b.end_dt
            
            # Assicurati che durata_extra_min sia definita anche se non ci sono extra
            if 'durata_extra_min' not in locals():
                durata_extra_min = 0
            
            # Festivo: +30% su tutto (Base + Extra + Notturno)
        ...
        tour_operator_val = b.tour_operator_originale if b.tour_operator_originale else "Caboverdetime"
        ...
        rows_detail.append({
            ...
            "INIZIO_DT": start_caboverdetime if not pd.isna(start_caboverdetime) else b.start_dt,
    ...
    month_row = pd.DataFrame([{
        "TOUR OPERATOR": "Caboverdetime" if "TOUR OPERATOR" in detail_df.columns else "",
        "PERIODO": "MESE",
...
def main():
    parser = argparse.ArgumentParser(description="Calcolatore blocchi Caboverdetime")
    ...
    parser.add_argument("--to", default="caboverdetime", help="Keyword tour operator (default: caboverdetime)")
