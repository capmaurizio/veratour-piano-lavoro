...
def _build_errore(errore: Optional[str], nota: Optional[str]) -> str:
    """Compone il campo ERRORE unendo eventuale errore di calcolo e nota dal file sorgente."""
    parts = []
...
def process_files(input_files: List[str], cfg: CalcConfig) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    for file_path in input_files:
        for sheet_name, sdf0 in iter_excel_sheets(file_path):
            ...
            # Minimal cols: DATA+APT sempre + schema orario (nuovo 2026 o vecchio TURNO)
            has_orario_pf = (cols.get("inizio_turno") and cols.get("fine_turno")) or cols.get("turno")
            if not cols["data"] or not cols["apt"] or not has_orario_pf:
                continue
            ...
            for idx, r in sdf.iterrows():
                ...
                if key not in blocks:
                    ...
                    # Extract nota from first row (point of attention from source file)
...
def create_assistenti_vrn_sheet(detail_df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        # Per Alpitour, usa NOTTE_MIN_RAW se disponibile
        notte_min_total = df_vrn['NOTTE_MIN_RAW'].sum() if 'NOTTE_MIN_RAW' in df_vrn.columns else (df_vrn['NOTTE_MIN'].sum() if 'NOTTE_MIN' in df_vrn.columns else 0)
