...
Esempi:
  python consuntivoveratour.py -i "Piano lavoro DICEMBRE 25.xlsx" -o "OUT_DIC.xlsx"
  python consuntivoveratour.py -i file1.xlsx file2.xlsx --apt VRN --round-extra CEIL --round-extra-step 5
"""
...
def parse_time_value(x) -> Optional[Tuple[int, int]]:
    ...
    - pandas Timestamp
    - floats (Excel times)
    - strings 'H', 'HH', 'H:MM', 'HH.MM', 'HH:MM:SS', 'HH;MM'
    """
...
def parse_turno(turno_raw: str) -> Tuple[Optional[str], Optional[str], bool, str]:
    ...
    # PRIMA: Gestione punti e punti e virgola come separatore tra orari (PRIMA di controllare turni incompleti)
    # Pattern tipo "13:30.17:00" o "04:30 . 07:30" dove punto/punto e virgola separano due orari
    # Convertiamo "HH:MM . HH:MM" o "HH:MM.HH:MM" in "HH:MM-HH:MM"
    s2 = re.sub(r'(\d{1,2}[:;]\d{1,2})\s*[\.;]\s*(\d{1,2}[:;]\d{1,2})', r'\1-\2', s2)
    
    # Poi convertiamo i punti e punti e virgola rimasti in due punti (per gli orari tipo "8.30" -> "8:30" o "13;15" -> "13:15")
    s2 = s2.replace(".", ":")
    s2 = s2.replace(";", ":")
    
    # Gestione doppio trattino tipo "-DEC" dopo un trattino (es: "03:00- 06:30 -DEC")
    # Rimuoviamo spazi e trattini extra prima di DEC
    s2 = re.sub(r'-\s*-', '-', s2)  # Doppio trattino con spazio
    s2 = re.sub(r'-\s*DEC', ' DEC', s2, flags=re.IGNORECASE)  # Trattino prima di DEC
    s2 = re.sub(r'-\s*dec', ' dec', s2, flags=re.IGNORECASE)
    
    # DOPO la normalizzazione, controlla se c'è già un range completo
    m = TIME_RANGE_RE.search(s2)
    if m:
        # C'è un range completo, procedi normalmente
        pass
    else:
        # Gestione turni incompleti tipo "20:25-DEC" (manca orario fine)
        # Cerca pattern HH:MM seguito da trattino e testo (DEC, ecc.) senza altro orario
        incomplete_pattern = re.compile(rf"^.*?(\d{{1,2}})[:\.](\d{{1,2}})\s*{TIME_SEP_PATTERN}\s*([A-Za-z]+)(?!.*\d{{1,2}}[:\.]\d{{1,2}})", re.IGNORECASE)
        m_incomplete = incomplete_pattern.search(s2)
        if m_incomplete:
            h1, m1 = m_incomplete.group(1), m_incomplete.group(2)
            h1 = int(h1)
            ...
            s_norm = s2[:m_incomplete.start()] + norm_range + s2[m_incomplete.end():]
    ...
    # Se non abbiamo già trovato un match, riprova dopo tutte le normalizzazioni
    if not m:
        m = TIME_RANGE_RE.search(s2)
    if not m:
        # Tentativo di gestire turni incompleti tipo "20:25-DEC" (manca orario fine)
        # Cerca un orario seguito da trattino e testo (DEC, ecc.) o solo trattino
        # Pattern: HH:MM seguito da trattino e testo (non un altro orario)
        single_time_with_text = re.compile(rf"({TIME_TOKEN})\s*{TIME_SEP_PATTERN}\s*([A-Za-z]+)", re.IGNORECASE)
        m_single = single_time_with_text.search(s2)
        if m_single:
            ...
            if no_dec:
                s_norm = NO_DEC_RE.sub("NO DEC", s_norm)
            return (start, end, no_dec, s_norm)
        # Se non trovato, prova pattern più semplice: HH:MM seguito da trattino (senza altro orario)
        simple_time = re.compile(rf"^.*?({TIME_TOKEN})\s*{TIME_SEP_PATTERN}(?!\s*\d)", re.IGNORECASE)
        m_simple = simple_time.search(s2)
        if m_simple:
            h1, m1 = m_simple.group(1), m_simple.group(2)
            h1 = int(h1)
            m1 = int(m1) if m1 is not None else 0
            start = f"{h1:02d}:{m1:02d}"
            end = "00:00"
            return (start, end, no_dec, s2)
...
def _create_assistenti_vrn_sheet_legacy(df_vrn: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in df_vrn.iterrows():
        ...
        # Estrai TOUR OPERATOR se presente
        tour_operator = row.get('TOUR OPERATOR', '') if 'TOUR OPERATOR' in row.index else ''
    ...
    # Raggruppa per TOUR OPERATOR e ASSISTENTE se TOUR OPERATOR è presente
    groupby_cols = ['ASSISTENTE']
    if 'TOUR OPERATOR' in df_calc.columns and df_calc['TOUR OPERATOR'].notna().any():
        groupby_cols = ['TOUR OPERATOR', 'ASSISTENTE']
    
    assistenti_totals = df_calc.groupby(groupby_cols).agg({
        'BASE_EUR': 'sum',
        'EXTRA_EUR': 'sum',
        'EXTRA_MIN': 'sum',
        'NOTTE_EUR': 'sum',
    ...
    # Estrai indici
    if isinstance(assistenti_totals.index, pd.MultiIndex) or len(groupby_cols) > 1:
        # MultiIndex o più colonne: estrai TOUR OPERATOR e ASSISTENTE
        if 'TOUR OPERATOR' in assistenti_totals.columns:
            assistenti_totals.columns = ['TOUR OPERATOR', 'Assistente', 'Turno (€)', 'Extra (€)', 'Extra (min)', 'Notturno (€)', 'Notturno (min)', 'TOTALE (€)', 'Blocchi']
        else:
...
def write_output_excel(output_path: str, detail_df: pd.DataFrame, totals_df: pd.DataFrame, discr_df: pd.DataFrame) -> None:
    with pd.ExcelWriter(output_path, engine="openpyxl", datetime_format="YYYY-MM-DD HH:MM") as writer:
        ...
        if not discr_df.empty:
            ...
            # Crea DataFrame vuoto con colonne corrette incluso TOUR OPERATOR
