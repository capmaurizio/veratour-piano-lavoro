...
"""
Domina — Calcolatore blocchi

Cosa fa:
- Legge file Excel "Piano Lavoro" (anche con più fogli)
- Filtra TO=Domina e APT richiesto (opzionale)
...
Esempi:
  python consuntivodomina.py -i "Piano lavoro DICEMBRE 25.xlsx" -o "OUT_DOMINA.xlsx"
  python consuntivodomina.py -i "Piano lavoro DICEMBRE 25.xlsx" -o "OUT_DOMINA.xlsx" --apt VRN
"""
...
# Tariffe base Domina per 2h30 (150 min)
TARIFFE_BASE_DOMINA = {
    "BGY": 80.0,
    "VRN": 85.0,
    "BLQ": 85.0,
...
# Tariffe notturne Domina (€/min) - 20% della tariffa base oraria (calcolo "puro")
TARIFFE_NOTTE_DOMINA = {
    "BGY": 0.1067,  # 80€/2.5h = 32€/h, 20% = 6.40€/h = 0.1067€/min
    "VRN": 0.1133,  # 85€/2.5h = 34€/h, 20% = 6.80€/h = 0.1133€/min
    "BLQ": 0.1133,  # 85€/2.5h = 34€/h, 20% = 6.80€/h = 0.1133€/min
    "MXP": 0.1200,  # 90€/2.5h = 36€/h, 20% = 7.20€/h = 0.1200€/min
    "NAP": 0.1200,  # 90€/2.5h = 36€/h, 20% = 7.20€/h = 0.1200€/min
    "BRI": 0.1200,  # 90€/2.5h = 36€/h, 20% = 7.20€/h = 0.1200€/min
    "CTA": 0.1200,  # 90€/2.5h = 36€/h, 20% = 7.20€/h = 0.1200€/min
    "PMO": 0.1200,  # 90€/2.5h = 36€/h, 20% = 7.20€/h = 0.1200€/min
    "FCO": 0.1200,  # 90€/2.5h = 36€/h, 20% = 7.20€/h = 0.1200€/min
    "VCE": 0.1333,  # 100€/2.5h = 40€/h, 20% = 8.00€/h = 0.1333€/min
    "PSI": 0.1333,  # 100€/2.5h = 40€/h, 20% = 8.00€/h = 0.1333€/min
}
...
def night_minutes(start_dt: pd.Timestamp, end_dt: pd.Timestamp) -> int:
    """
    Calcola minuti notturni nella fascia 22:00-06:00 (Domina)
...
def get_base_tariffa(apt: str) -> float:
    """Restituisce la tariffa base per l'aeroporto (per 2h30)"""
    apt_upper = apt.upper().strip()
    return TARIFFE_BASE_DOMINA.get(apt_upper, 90.0)  # Default: 90€


def get_night_tariffa(apt: str) -> float:
    """Restituisce la tariffa notturna per minuto per l'aeroporto"""
    apt_upper = apt.upper().strip()
    return TARIFFE_NOTTE_DOMINA.get(apt_upper, 0.1200)  # Default: 90€ rate


def compute_park_eur(apt: str, durata_ore: float) -> float:
    """
    Calcola spese parcheggio: solo per NAP e BRI
    - NAP (Napoli): €1 all'ora
    - BRI (Bari): €6 ogni 3 ore (arrotondato per eccesso)
    """
    apt_upper = apt.upper().strip()
    if apt_upper == "NAP":
        # Napoli: €1 all'ora
        return durata_ore * 1.0
    elif apt_upper == "BRI":
        # Bari: €6 ogni 3 ore (arrotondato per eccesso)
        # Esempio: 1h = 1 blocco (€6), 3h = 1 blocco (€6), 3h01 = 2 blocchi (€12), 6h = 2 blocchi (€12)
        blocchi_3h = int(np.ceil(durata_ore / 3.0))
        return blocchi_3h * 6.0
    return 0.0
...
def detect_columns(df: pd.DataFrame) -> Dict[str, Optional[str]]:
    ...
    return {
        ...
        "parcheggio": find_col(df, [r"^PARCHEGGIO$", r"^PARK$", r"\bPARCHEGGIO\b"]),  # Per spese parcheggio NAP e BRI
...
class BlockAgg:
    ...
    parcheggio: Optional[float] = None  # Spese parcheggio per NAP e BRI
...
def process_files(input_files: List[str], cfg: CalcConfig) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    for file_path in input_files:
        for sheet_name, sdf0 in iter_excel_sheets(file_path):
            ...
            # Filter TO if present - Domina
            if cols["tour_operator"]:
                mask_domina = sdf[cols["tour_operator"]].astype(str).str.contains("domina", case=False, na=False)
                sdf = sdf[mask_domina].copy()
                if sdf.empty:
                    continue
            ...
            for idx, r in sdf.iterrows():
                ...
                # Estrai spese parcheggio (solo per NAP e BRI)
                parcheggio_val = None
                if cols["parcheggio"] and cols["parcheggio"] in r.index:
                    park_val = r[cols["parcheggio"]]
                    try:
                        if pd.notna(park_val):
                            parcheggio_val = float(park_val)
                    except:
                        pass
                # Se non c'è colonna PARCHEGGIO, il calcolo verrà fatto dopo quando avremo la durata effettiva
                # Per ora lasciamo None, verrà calcolato nel blocco di elaborazione
                # if parcheggio_val is None:
                #     if apt.upper() in ["NAP", "BRI"]:
                #         parcheggio_val = compute_park_eur(apt, durata_ore)  # Calcolato dopo con durata effettiva

                # Extract ATD candidates from row (prima per poterli usare nel placeholder se TURNO è vuoto)
                ...
                # Extract STD candidates from row (prima per poterli usare nel placeholder se TURNO è vuoto)
                ...
                # DOMINA: ogni riga del Piano Lavoro genera un blocco separato
                ...
                if key in blocks:
                    ...
                    blocks[key] = BlockAgg(
                        ...
                        parcheggio=parcheggio_val if parcheggio_val else None,
                    )
    ...
    for key, b in sorted(blocks.items(), key=lambda kv: (kv[1].date, kv[1].apt, kv[1].first_source.original_order)):
        ...
        if std_sel is None:
            ...
            start_domina = b.start_dt
            errore_msg = "STD non disponibile, uso TURNO come fallback"
        else:
            # START = STD - 2:30
            start_domina = std_sel - pd.Timedelta(hours=2, minutes=30)
            errore_msg = None
        
        # END = ATD (se disponibile) o fine turno
        atd_sel = b.atd_list[0] if b.atd_list else None
        if atd_sel is not None:
            end_domina = atd_sel
        else:
            # Se non c'è ATD, usa fine turno
            end_domina = b.end_dt
            if errore_msg:
                errore_msg = f"{errore_msg}; ATD non disponibile, uso fine turno"
            else:
                errore_msg = "ATD non disponibile, uso fine turno"
        
        # Verifica che start_domina e end_domina siano validi
        if pd.isna(start_domina) or pd.isna(end_domina):
            ...
            durata_effettiva_min = int((end_domina - start_domina).total_seconds() / 60.0)
            
            # Turno base (sempre 2h30 = 150 min)
            turno_eur = compute_turno_eur(b.apt)
            
            # Extra = MAX(0, durata_effettiva - 150)
            extra_eur = compute_extra_eur(durata_effettiva_min, cfg)
            extra_min = max(0, durata_effettiva_min - cfg.durata_base_min)
            
            # Notturno (22:00-06:00)
            night_min = night_minutes(start_domina, end_domina)
            night_eur = compute_night_eur(night_min, b.apt)
            
            # Spese parcheggio (solo per NAP e BRI)
            # Calcola durata in ore per il parcheggio
            durata_ore = durata_effettiva_min / 60.0
            park_eur = b.parcheggio if b.parcheggio is not None else compute_park_eur(b.apt, durata_ore)
            
            # Festivo: +30% su tutto (Base + Extra + Notte + Park)
            ...
            if is_festivo:
                totale = (turno_eur + extra_eur + night_eur + park_eur) * cfg.festivo_multiplier
            else:
                totale = turno_eur + extra_eur + night_eur + park_eur
        
        # Usa TOUR OPERATOR originale salvato nel blocco
        tour_operator_val = b.tour_operator_originale if b.tour_operator_originale else "Domina"
        ...
        rows_detail.append({
            ...
            "INIZIO_DT": start_domina if not pd.isna(start_domina) else b.start_dt,
            "FINE_DT": end_domina if not pd.isna(end_domina) else b.end_dt,
            ...
            "PARCHEGGIO_EUR": round(park_eur, 2),
    ...
    month_row = pd.DataFrame([{
        "TOUR OPERATOR": "Domina" if "TOUR OPERATOR" in detail_df.columns else "",
        "PERIODO": "MESE",
...
def create_apt_detail_sheet(df_apt: pd.DataFrame) -> pd.DataFrame:
    ...
    # Aggiungi Parcheggio se presente
    if 'PARCHEGGIO_EUR' in df_apt.columns:
        output_cols['Parcheggio (€)'] = df_apt['PARCHEGGIO_EUR'].round(2)
    ...
    # Aggiungi Parcheggio se presente
    if 'PARCHEGGIO_EUR' in df_apt.columns:
        total_row_dict['Parcheggio (€)'] = df_apt['PARCHEGGIO_EUR'].sum()
...
def main():
    parser = argparse.ArgumentParser(description="Calcolatore blocchi Domina")
    ...
    parser.add_argument("--to", default="domina", help="Keyword tour operator (default: domina)")
