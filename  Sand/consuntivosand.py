...
"""
SAND — Calcolatore blocchi

Cosa fa:
- Legge file Excel "Piano Lavoro" (anche con più fogli)
- Filtra TO=SAND e APT richiesto (opzionale)
...
Esempi:
  python consuntivosand.py -i "Piano lavoro DICEMBRE 25.xlsx" -o "OUT_SAND.xlsx"
  python consuntivosand.py -i "Piano lavoro DICEMBRE 25.xlsx" -o "OUT_SAND.xlsx" --apt VRN
"""
...
# Tariffe base SAND (aggiornate da CalcoloSand.txt)
# Placeholder: usando tariffe Domina come base
TARIFFE_BASE_SAND = {
    "BGY": 65.0,
    "VRN": 70.0,
    "BLQ": 70.0,
...
# Tariffe notturne SAND (€/min) - aggiornate da CalcoloSand.txt
# Placeholder: usando tariffe Domina come base (20% della tariffa base oraria)
TARIFFE_NOTTE_SAND = {
    "BGY": 0.09,   # €5,40/h = €0,09/min
    "VRN": 0.10,   # €6,00/h = €0,10/min
    "BLQ": 0.10,   # €6,00/h = €0,10/min
    "NAP": 0.10,   # €6,00/h = €0,10/min
    "FCO": 0.10,   # €6,00/h = €0,10/min
    "VCE": 0.10,   # €6,00/h = €0,10/min
}
...
class CalcConfig:
    ...
    # Tariffe SAND (da aggiornare quando CalcoloSand.txt sarà completo)
    rate_extra_per_h: float = 18.0  # €18/h = €0.30/min (default, da verificare)
    durata_base_min: int = 150  # 2h30 = 150 minuti (default, da verificare)
    festivo_multiplier: float = 1.30  # +30% per festivi (default, da verificare)
...
def night_minutes(start_dt: pd.Timestamp, end_dt: pd.Timestamp) -> int:
    """
    Calcola minuti notturni nella fascia 22:00-03:59 (SAND)
    Gestisce attraversamento mezzanotte
    Nota: tecnicamente gestito come [22:00, 04:00) che è equivalente a 22:00–03:59
    """
    ...
    while current < end_dt:
        ...
        # Fascia: 22:00-03:59 (SAND) - gestito come hour < 4 (esclude 04:00)
        is_night = False
        if hour == 22:
            is_night = True
        elif hour == 23:
...
def get_base_tariffa(apt: str) -> float:
    """Restituisce la tariffa base per l'aeroporto (aggiornata da CalcoloSand.txt)"""
    apt_upper = apt.upper().strip()
    return TARIFFE_BASE_SAND.get(apt_upper, 90.0)  # Default: 90€


def get_night_tariffa(apt: str) -> float:
    """Restituisce la tariffa notturna per minuto per l'aeroporto (aggiornata da CalcoloSand.txt)"""
    apt_upper = apt.upper().strip()
    return TARIFFE_NOTTE_SAND.get(apt_upper, 0.1200)  # Default: 90€ rate
...
def compute_extra_eur(durata_effettiva_min: int, cfg: CalcConfig) -> float:
    """
    Calcola importo extra: MAX(0, durata_effettiva - 150) × €0.30/min
    NOTA: Per SAND questa funzione non viene utilizzata (extra sempre 0)
    """
...
def compute_night_eur(night_min: int, apt: str) -> float:
    """Calcola importo notturno: night_min × tariffa_notte_per_min"""
...
def detect_columns(df: pd.DataFrame) -> Dict[str, Optional[str]]:
    ...
    return {
        ...
        "convocazione": find_col(df, [r"^CONVOCAZIONE$", r"^CONV\.NE$", r"\bCVC\b", r"^CONV\b"]),
    }
...
def process_files(input_files: List[str], cfg: CalcConfig) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """
    Legge e processa i file Piano Lavoro per Domina
    ...
    for file_path in input_files:
        for sheet_name, sdf0 in iter_excel_sheets(file_path):
            ...
            # Filter TO if present - SAND
            if cols["tour_operator"]:
                mask_sand = sdf[cols["tour_operator"]].astype(str).str.contains("sand", case=False, na=False)
                sdf = sdf[mask_sand].copy()
                if sdf.empty:
                    continue
            ...
            for idx, r in sdf.iterrows():
                ...
                # Extract CONVOCAZIONE (CVC) from row - SAND
                cvc_dt = None
                if cols.get("convocazione"):
                    cvc_val = r[cols["convocazione"]]
                    if pd.notna(cvc_val):
                        # Parse CONVOCAZIONE come orario
                        cvc_parsed = parse_time_value(cvc_val)
                        if cvc_parsed:
                            hh, mm = cvc_parsed
                            # Calcola cvc_dt correttamente usando Timestamp.combine
                            cvc_dt = pd.Timestamp.combine(d, pd.Timestamp(f"{hh:02d}:{mm:02d}").time())
                            # Per SAND, CVC è relativo alla data della riga
                            # (il notturno gestirà correttamente il caso CVC tra 00:00 e 03:59)
                ...
                if key in blocks:
                    ...
                    if pd.isna(b.cvc_dt) and pd.notna(cvc_dt):
                        b.cvc_dt = cvc_dt
                else:
                    blocks[key] = BlockAgg(
                        date=d,
    ...
    for key, b in sorted(blocks.items(), key=lambda kv: (kv[1].date, kv[1].apt, kv[1].first_source.original_order)):
        # SAND: START = STD - 2:30 (da verificare nelle regole)
        # Se STD non disponibile, usa TURNO come fallback o segnala errore
        std_sel = b.std_list[0] if b.std_list else None
        
        if std_sel is None:
            # Se non c'è STD, usa il TURNO come fallback (ma segnala)
            start_sand = b.start_dt
            errore_msg = "STD non disponibile, uso TURNO come fallback"
        else:
            # START = STD - 2:30 (da verificare nelle regole)
            start_sand = std_sel - pd.Timedelta(hours=2, minutes=30)
            errore_msg = None
        
        # END = STD (regole aggiornate SAND)
        # Le regole aggiornate specificano che END = STD (non più ATD)
        # Per SAND, assicuriamoci che STD sia nella data corretta della riga
        if std_sel is not None:
            # Se std_sel è del giorno successivo ma dovrebbe essere del giorno corrente,
            # correggilo usando la data della riga
            if b.date and std_sel.date() != b.date.date():
                # Ricalcola STD usando la data della riga
                std_hour = std_sel.hour
                std_minute = std_sel.minute
                end_sand = pd.Timestamp.combine(b.date.date(), pd.Timestamp(f"{std_hour:02d}:{std_minute:02d}").time())
            else:
                end_sand = std_sel
            if errore_msg:
                errore_msg = f"{errore_msg}; END = STD"
        else:
            # Se non c'è STD, segnala errore
            end_sand = pd.NaT
            if errore_msg:
                errore_msg = f"{errore_msg}; STD non disponibile per END"
            else:
                errore_msg = "STD non disponibile per END"
        
        # Verifica che start_sand e end_sand siano validi
        if pd.isna(start_sand) or pd.isna(end_sand):
            errore_msg = "Impossibile calcolare START o END"
            turno_eur = 0.0
            extra_min = 0
            extra_eur = 0.0
            night_min = 0
            night_eur = 0.0
            totale = 0.0
            durata_effettiva_min = 0
        else:
            # Durata effettiva = END - START
            # Per SAND: START = CVC (convocazione), END = STD
            # Se CVC disponibile, usa CVC come START, altrimenti usa start_sand (STD - 2:30)
            if pd.notna(b.cvc_dt):
                start_for_durata = b.cvc_dt
            else:
                start_for_durata = start_sand
            durata_effettiva_min = int((end_sand - start_for_durata).total_seconds() / 60.0)
            
            # Turno base (da verificare nelle regole)
            turno_eur = compute_turno_eur(b.apt)
            
            # SAND: Nessun calcolo di ore extra (sempre 0)
            # Il pacchetto base di 2h30 è fisso, indipendentemente dalla durata effettiva
            extra_eur = 0.0
            extra_min = 0
            
            # Notturno (22:00-03:59) - SAND
            # IMPORTANTE: Il notturno viene calcolato dall'orario CVC (convocazione) in poi
            # CVC = b.cvc_dt (orario di CONVOCAZIONE dalla colonna, non dal TURNO)
            # START = CVC, END = STD (regole aggiornate)
            # Eventuali fasce notturne precedenti al CVC non devono essere considerate
            cvc_dt = b.cvc_dt  # Orario di convocazione dalla colonna CONVOCAZIONE
            

            # Per SAND, CVC è obbligatorio per il calcolo del notturno
            # Se CVC non disponibile, non calcoliamo il notturno (0 minuti)
            # Inizializza night_min a 0 per sicurezza
            night_min = 0
            if pd.notna(cvc_dt) and pd.notna(end_sand):
                # Il notturno decorre solo a partire dall'orario CVC
                # START = CVC, END = STD (come specificato nelle regole aggiornate)
                # Calcola notturno da CVC a STD (end_sand = STD)
                night_min = night_minutes(cvc_dt, end_sand)
                if b.assistente and "Leonia" in str(b.assistente) and b.date and "27" in str(b.date):
                    print(f"DEBUG Leonia 27/12: night_min={night_min}")
            night_eur = compute_night_eur(night_min, b.apt)
            
            # Festivo: +30% su tutto (Base + Notte) - SAND non ha extra
            ...
            if is_festivo:
                totale = (turno_eur + night_eur) * cfg.festivo_multiplier
            else:
                totale = turno_eur + night_eur
        
        # Usa TOUR OPERATOR originale salvato nel blocco
        tour_operator_val = b.tour_operator_originale if b.tour_operator_originale else "SAND"
        ...
        rows_detail.append({
            ...
            "INIZIO_DT": start_sand if not pd.isna(start_sand) else b.start_dt,
    ...
    month_row = pd.DataFrame([{
        "TOUR OPERATOR": "SAND" if "TOUR OPERATOR" in detail_df.columns else "",
        "PERIODO": "MESE",
...
def main():
    parser = argparse.ArgumentParser(description="Calcolatore blocchi SAND")
    ...
    parser.add_argument("--to", default="sand", help="Keyword tour operator (default: sand)")
