...
"""
MichelTours — Calcolatore blocchi

Cosa fa:
- Legge file Excel "Piano Lavoro" (anche con più fogli)
- Filtra TO=MichelTours e APT richiesto (opzionale)
...
Esempi:
  python consuntivomicheltours.py -i "Piano lavoro DICEMBRE 25.xlsx" -o "OUT_MICHELTOURS.xlsx"
  python consuntivomicheltours.py -i "Piano lavoro DICEMBRE 25.xlsx" -o "OUT_MICHELTOURS.xlsx" --apt BGY
"""
...
# Tariffe base MichelTours per 3h (180 min)
TARIFFE_BASE_MICHELTOURS = {
    "BGY": 85.0,
    "MXP": 90.0,
    "VCE": 90.0,
}

# Tariffe notturne MichelTours (€/min) - 20% della tariffa base oraria (calcolo "puro")
# Base 3h: BGY=85€/3h=28.33€/h, MXP/VCE=90€/3h=30€/h
TARIFFE_NOTTE_MICHELTOURS = {
    "BGY": 0.0944,  # 85€/3h = 28.33€/h, 20% = 5.67€/h = 0.0944€/min (≈ 0.095)
    "MXP": 0.1000,  # 90€/3h = 30€/h, 20% = 6.00€/h = 0.1000€/min
    "VCE": 0.1000,  # 90€/3h = 30€/h, 20% = 6.00€/h = 0.1000€/min
}
...
class CalcConfig:
    ...
    to_keyword: str = "micheltours"  # Default
    ...
    durata_base_min: int = 180  # 3h = 180 minuti
...
def night_minutes(start_dt: pd.Timestamp, end_dt: pd.Timestamp) -> int:
    """
    Calcola minuti notturni nella fascia 22:00-06:00 (MichelTours)
...
def get_base_tariffa(apt: str) -> float:
    """Restituisce la tariffa base per l'aeroporto (per 3h)"""
    apt_upper = apt.upper().strip()
    return TARIFFE_BASE_MICHELTOURS.get(apt_upper, 90.0)  # Default: 90€


def get_night_tariffa(apt: str) -> float:
    """Restituisce la tariffa notturna per minuto per l'aeroporto"""
    apt_upper = apt.upper().strip()
    return TARIFFE_NOTTE_MICHELTOURS.get(apt_upper, 0.1000)  # Default: MXP/VCE rate


def compute_turno_eur(apt: str) -> float:
    """Calcola importo turno base (sempre 3h = 180 min)"""
    return get_base_tariffa(apt)


def compute_extra_eur(durata_effettiva_min: int, cfg: CalcConfig) -> float:
    """
    Calcola importo extra: MAX(0, durata_effettiva - 180) × €0.30/min
    """
...
def process_files(input_files: List[str], cfg: CalcConfig) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """
    Legge e processa i file Piano Lavoro per MichelTours
    ...
    for file_path in input_files:
        for sheet_name, sdf0 in iter_excel_sheets(file_path):
            ...
            for idx, r in sdf.iterrows():
                ...
                # MICHELTOURS: ogni riga del Piano Lavoro genera un blocco separato
    ...
    for key, b in sorted(blocks.items(), key=lambda kv: (kv[1].date, kv[1].apt, kv[1].first_source.original_order)):
        # MICHELTOURS: START = STD - 2:30 (come Domina/Baobab)
        ...
        if pd.isna(start_micheltours) or pd.isna(end_micheltours):
            ...
            durata_effettiva_min = int((end_micheltours - start_micheltours).total_seconds() / 60.0)
            
            # Turno base (sempre 3h = 180 min)
            turno_eur = compute_turno_eur(b.apt)
            
            # Extra = MAX(0, durata_effettiva - 180)
            extra_eur = compute_extra_eur(durata_effettiva_min, cfg)
            extra_min = max(0, durata_effettiva_min - cfg.durata_base_min)
            
            # Notturno (22:00-06:00)
            night_min = night_minutes(start_micheltours, end_micheltours)
            night_eur = compute_night_eur(night_min, b.apt)
            
            # Festivo: +30% su tutto (Base + Extra + Notte)
        ...
        tour_operator_val = b.tour_operator_originale if b.tour_operator_originale else "MichelTours"
        ...
        rows_detail.append({
            ...
            "INIZIO_DT": start_micheltours if not pd.isna(start_micheltours) else b.start_dt,
            "FINE_DT": end_micheltours if not pd.isna(end_micheltours) else b.end_dt,
    ...
    month_row = pd.DataFrame([{
        "TOUR OPERATOR": "MichelTours" if "TOUR OPERATOR" in detail_df.columns else "",
        "PERIODO": "MESE",
...
def main():
    parser = argparse.ArgumentParser(description="Calcolatore blocchi MichelTours")
    ...
    parser.add_argument("--apt", nargs="+", help="Filtra aeroporti (es: --apt BGY MXP)")
    parser.add_argument("--to", default="micheltours", help="Keyword tour operator (default: micheltours)")
